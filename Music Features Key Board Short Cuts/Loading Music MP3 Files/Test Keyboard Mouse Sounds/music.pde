...
void musicLoadSetup() {
  ...
  String musicPathway = soundPathway + fslash + SoundEffect;
  String soundEffectPathway = soundPathway + fslash + SoundEffect;
  //Inspect Variable 
  println (musicPathway);
  println (soundEffectPathway);
  //
  minim = new Minim(this); //load from data directory, loadFile should also from project folder, like loadImage
  //
  ...
  soundEffect0 = minim.loadFile( soundEffecctPathway + fslash + CarDoorClosing + dereferencer + fileExtension );
